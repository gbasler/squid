...
maxMaybe1 = \ds -> let
  rec π' = let
        ...
        _1 = (let (:) arg _ = π' in arg) > (case ψ'3 of { Just ρ'16 -> (case _3 of { True -> (let (:) arg _ = (let (:) _ arg = π' in arg) in arg); False -> ρ'16 }); Nothing -> (let (:) arg _ = (let (:) _ arg = π' in arg) in arg) })
        _4 = Just (let (:) arg _ = (let (:) _ arg = π' in arg) in arg)
        ψ'2 = case sel6 rec_call' of { (:) ρ'13 ρ'14 -> (case sel5 rec_call' of { Just ρ'15 -> (case _3 of { True -> Just (let (:) arg _ = (let (:) _ arg = π' in arg) in arg); False -> ψ'3 }); Nothing -> _4 }); [] -> _4 }
        ψ' = case (let (:) _ arg = π' in arg) of { (:) ρ'11 ρ'12 -> ψ'2; [] -> Nothing }
