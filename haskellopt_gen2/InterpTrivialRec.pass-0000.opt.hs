...
-- Field reductions: 20
-- Case commutings:  0
-- Total nodes: 313; Boxes: 126; Branches: 126
-- Apps: 20; Lams: 1
...
exec = \pgm -> let
  rec _fε'2 = let
        rec'4 pgm'2 = case pgm'2 of { (:) ρ'20 ρ'21 -> True : (rec ρ'21); [] -> False : (case pgm of { (:) ρ'22 ρ'23 -> True : (rec ρ'23); [] -> False : (rec'4 pgm) }) }
        rec'5 pgm'3 = case pgm'3 of { (:) ρ'28 ρ'29 -> True : (case ρ'29 of { (:) ρ'30 ρ'31 -> True : (rec ρ'31); [] -> False : (case pgm of { (:) ρ'32 ρ'33 -> True : (rec ρ'33); [] -> False : (rec'5 pgm) }) }); [] -> False : (case pgm of { (:) ρ'34 ρ'35 -> True : (case ρ'35 of { (:) ρ'36 ρ'37 -> True : (rec ρ'37); [] -> False : (rec'5 pgm) }); [] -> False : (rec'5 pgm) }) }
        in case _fε'2 of { (:) ρ'24 ρ'25 -> True : (case ρ'25 of { (:) ρ'26 ρ'27 -> True : (rec ρ'27); [] -> False : (rec'4 pgm) }); [] -> False : (rec'5 pgm) }
  rec' pgm' = let
