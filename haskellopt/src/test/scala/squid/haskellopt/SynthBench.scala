  ...
  def genPgrm(sizes: Iterable[Int], fileName: String, bench: Bool, exportOnlyMain: Bool, forceInline: Bool) = {
    ...
    val defs = for {
      ...
      prod =
        ...
        s"prod_$size :: Num a => [a] -> a\n" +
        //s"prod_$size [${xs mkString ","}] = ${
        //  (xs zip ys).foldLeft("0"){case (acc,xy) => acc + " + " + xy._1}}"
        (xs.reverse.tails.map { xsr =>
          val xs = xsr.reverse
          s"prod_$size [${xs mkString ","}] = ${
            (xs zip ys).foldLeft("0"){case (acc,xy) => acc + " + " + xy._1}}"
        }).mkString("\n")
...
object MeasureSynthBenchSizes extends BenchTests with App {
  ...
  val coreSizes = for {
    ...
    def termsFor(forceInline: Bool) = {
      ...
      %("ghc", "-O2", "-fforce-recomp", "-ddump-to-file", "-ddump-prep", path)(pwd)
    ...
    %("ghc", "-O2", "-fforce-recomp", "-ddump-to-file", "-ddump-prep", path2)(pwd)
