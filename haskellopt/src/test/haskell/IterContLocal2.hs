...
-- FIXME minimized example below where things don't fully reduce

-- main = exitSuccess

-- -- FIXME minimized example where things don't fully reduce
-- (count, nats) =
--     -- ( loop (\k s -> k (s + 1)) 0
--     -- ( loop (\k s -> k s) 0
--     -- ( loop (\k -> id)
...
--     loop f state = rec state where
--         -- rec state = f (\new_state -> loop f new_state) state  -- FIXME later divergence when we remove outer `f` call
--         rec state = f rec state  -- FIXME later divergence when we remove outer `f` call
