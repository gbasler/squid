...
trait ASTReinterpreter { ast: AST =>
  ...
  trait Reinterpreter {
    ...
    protected def recv(bv: BoundVal) = newBase.bindVal(
      //bv.name optionUnless (_ startsWith "$") Else "x",
      if (bv.name startsWith "$") "x"+bv.name.tail else bv.name,
      // ^ bound variables starting with '$' are those that were not named explicitly in the source programs
      rect(bv.typ),
